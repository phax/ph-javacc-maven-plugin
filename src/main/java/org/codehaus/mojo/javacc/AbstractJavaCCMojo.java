...
public abstract class AbstractJavaCCMojo extends AbstractMojo
{
   ...
   * those that usually reside somewhere below "${basedir}/src" in the project structure. Files in
   * these source roots are owned by the user and must not be overwritten with generated files.
   */
  ...
  /**
   * This is a boolean option whose default value is <code>false</code>. Setting this option to
  ...
  /**
   * The name of a custom factory class used to create <code>Token</code> objects. This class must
  ...
  /**
   * The Java template type to be used. Must be "modern" or "classic". Defaults to "classic".
   *
   ...
   *
   * @return The file encoding of the output files or <code>null</code> if the user did not specify
   *         this mojo parameter.
   */
   ...
   *
   * @return The java template type or <code>null</code> if the user did not specify this mojo
   *         parameter.
   */
   ...
   *
   * @return The absolute path to the directory where the grammar files are located, never
   *         <code>null</code>.
   */
  ...
  public final void execute () throws MojoExecutionException, MojoFailureException
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (StringHelper.isEmpty (grammarEncoding))
        ...
        if (StringHelper.isEmpty (outputEncoding))
   ...
   *
   * @param grammarInfo
   *        The grammar info describing the grammar file to process, must not be <code>null</code>.
   * @throws MojoExecutionException
   *         If the invocation of the tool failed.
   * @throws MojoFailureException
   *         If the tool reported a non-zero exit code.
  ...
  /**
   * Scans the filesystem for output files and copies them to the specified compile source root. An
   * output file is only copied to the compile source root if it doesn't already exist in another
   * compile source root. This prevents duplicate class errors during compilation in case the user
   * provided customized files in <code>src/main/java</code> or similar.
   *
   * @param packageName
   *        The name of the destination package for the output files, must not be <code>null</code>.
   * @param sourceRoot
   *        The (absolute) path to the compile source root into which the output files should
   *        eventually be copied, must not be <code>null</code>.
   * @param tempDirectory
   *        The (absolute) path to the directory to scan for generated output files, must not be
   *        <code>null</code>.
   * @param updatePattern
   *        A glob pattern that matches the (simple) names of those files which should always be
   *        updated in case we are outputting directly into <code>src/main/java</code>, may be
   *        <code>null</code>. A leading "!" may be used to negate the pattern.
   * @throws MojoExecutionException
   ...
   * build directories. These compile source roots are assumed to contain hand-crafted sources that
   * must not be overwritten with generated files. In most cases, this is simply
  ...
  private void _determineNonGeneratedSourceRoots () throws MojoExecutionException
  {
    ...
    try
    {
      ...
      {
        File sourceRoot = new File (aElement);
   ...
   *
   * @param filename
   *        The source filename to check, relative to a source root, must not be <code>null</code>.
